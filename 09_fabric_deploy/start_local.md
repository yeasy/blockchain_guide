...
启动一个 Fabric 网络主要包括如下步骤。下面先给出的是 Fabric 2.x 时代常见的传统系统通道路径，后文会补充现代参与通道（participation）/无系统通道的思路：
...
* **启动 Orderer 节点**：传统做法是使用系统通道的初始区块文件启动排序服务，并由此创建系统通道；在 Fabric 2.3+ 的现代流程中，也可以采用参与通道（channel participation）方式，不必把系统通道当成唯一默认路径；
* **启动 Peer 节点**：不同的组织按照预置角色分别启动 Peer 节点；
* **创建通道**：传统做法是客户端使用新建应用通道的配置更新交易文件，向系统通道发送交易，创建新的应用通道；如果使用参与通道模式，则流程会不同；
